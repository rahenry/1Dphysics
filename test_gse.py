...
def tester(L, N, gamma, phi):
    flipZ = 0
    if phi == 0.5:
        flipZ = 1
    g = gamma * numpy.exp(phi*2.j*numpy.pi / N)
    flipZ = 0
    m1 = make_m1(L, N, g, flipZ)
    m2 = make_m2(L, N, g, flipZ)
    sol1 = scipy.linalg.eig(m1)
    sol2 = scipy.linalg.eig(m2)
    eigs1 = [e ** (1./N) * g/ L for e in sol1[0]]
    eigs2 = sol2[0]
    eigs1.sort()
    ...
    sign = ''
    if flipZ: sign = '-'
    print(f'L={L}, N={N}, gamma={gamma}, phi={phi}')
    e0 = 0
    ...
    gam = gamma / (1.+gamma)
    c = dict(N=N, L=L, gamma=gam, phi=phi, n_states=1, bc=0)
    s = System(c)
    s.prepare()
    eps_poly = find_eps_poly(s)
    p1 = []
    for p in eps_poly:
        p = p * s['lambda_scale']
        p = p / L
    ...
    spect1 = make_pf_spectrum2(e0, eigs1, N)
    spect_poly = make_pf_spectrum2(e0_poly, p1, N)
    for i in range(len(spect1)):
        print(spect1[i])
        print(spect_poly[i])
        if i > 10:
            break
